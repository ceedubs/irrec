...
import ceedubs.irrec.regex.RegexPrettyPrinter.{charClassCharsToEscape, nonCharClassCharsToEscape, specialNonCharClassCharToLit}
...
object Parser {
  ...
  def specialChar[_: P]: P[Char] =
    P(CharPred(specialNonCharClassCharToLit.contains(_)).!.map(s => specialNonCharClassCharToLit(s.head))
      .opaque(
